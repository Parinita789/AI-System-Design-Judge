 ...
 *
 * Output: agents/data/codebase-map/backend-api-flow.json
 *
 * Usage:
 *   cd agents/packages/api-flow && npm run extract -- [--repo-root <path>]
 */
...
function main(): void {
  ...
  const outDir = path.join(repoRoot, 'agents', 'data', 'codebase-map');

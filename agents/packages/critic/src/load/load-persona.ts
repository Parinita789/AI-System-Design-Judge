...
//   1. agents/packages/critic/personas/<name>.md (the shipped personas)
...
export function loadPersona(repoRoot: string, name: string): { text: string; resolvedPath: string } {
  ...
  throw new Error(
    `Persona '${name}' not found. Looked at ${shipped} and ${direct}. ` +
      `Drop a markdown file at agents/packages/critic/personas/${name}.md or pass an explicit path.`,
  );
}

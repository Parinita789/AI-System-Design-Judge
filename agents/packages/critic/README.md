...
(`agents/data/codebase-map/*.json`), the architecture / schema / api-flow
artifacts, and the source itself, then emits:

- per-module reviews under `agents/data/critic-out/per-module/<package>__<id>.md`
- a single synthesis report at `agents/data/critic-out/synthesis.md`
- a cross-run issue tracker at `agents/data/critic-out/issues.json`

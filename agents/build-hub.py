...
def discover_items() -> list[dict]:
    """Inspect agents/ and build a list of available views, grouped
    by category. Each item is { category, label, href, badge? }."""
    items: list[dict] = []

    # ---- Architecture: the curated end-to-end system diagram ----
    arch = AGENTS_DIR / 'architecture' / 'ARCHITECTURE.html'
    if arch.exists():
        items.append({
    ...
    # ---- Schema (Prisma ER) ----
    schema_html = AGENTS_DIR / 'schema' / 'SCHEMA_DIAGRAM.html'
    ...
    for pkg in ('backend',):
        api = AGENTS_DIR / 'graphify' / f'{pkg}-api-flow' / 'API_EXPLORER.html'
        if api.exists():
            items.append({
                'category': 'API flow',
    ...
    for pkg in ('backend', 'frontend', 'cli'):
        ex = AGENTS_DIR / 'graphify' / f'{pkg}-module-level' / 'MODULE_EXPLORER.html'
        if ex.exists():
            items.append({
                'category': 'Modules',
        ...
        d = AGENTS_DIR / 'graphify' / f'{pkg}-module-level'
    ...
    graphify_dir = AGENTS_DIR / 'graphify'
    if graphify_dir.is_dir():
        for pkg in ('backend', 'frontend', 'cli'):
            unified = graphify_dir / f'{pkg}-unified' / 'graphify-out'
    ...
    if graphify_dir.is_dir():
        for sub in sorted(graphify_dir.iterdir()):
            if not sub.is_dir():
                continue
            if (
...
def main() -> None:
    ...
    if not items:
        ...
        print('  agents/build-schema-diagram.py')
        print('  agents/graphify/build-mermaid.py backend')
        print('  agents/graphify/build-explorer.py backend')
        print('  cd agents/api-flow && npm run extract && cd ../.. && agents/graphify/build-api-explorer.py')

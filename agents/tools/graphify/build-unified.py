...
Usage:
  agents/tools/graphify/build-unified.py <package> [--level=high|file]

Examples:
  agents/tools/graphify/build-unified.py backend
  agents/tools/graphify/build-unified.py backend --level=file
"""
...
def find_module_files(repo_root: Path, package: str) -> list[str]:
    ...
    base = repo_root / 'agents' / 'data' / 'knowledge-graphs'
    return sorted(
...
def load_per_module(repo_root: Path, module: str, level: str) -> dict | None:
    ...
    path = repo_root / 'agents' / 'data' / 'knowledge-graphs' / module / 'graphify-out' / f'graph.{level}-level.json'
    if not path.is_file():
        return None
...
def build_unified(repo_root: Path, package: str, level: str) -> dict:
    mapper_path = repo_root / 'agents' / 'data' / 'codebase-map' / f'{package}.json'
...
def main() -> None:
    ...
    out_dir = REPO_ROOT / 'agents' / 'data' / 'knowledge-graphs' / f'{args.package}-unified'

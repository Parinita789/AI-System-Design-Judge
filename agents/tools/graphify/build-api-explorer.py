...
Renders the JSON emitted by agents/packages/api-flow/src/extract.ts as one
HTML page with:
...
Usage:
  agents/tools/graphify/build-api-explorer.py
  agents/tools/graphify/build-api-explorer.py --json <path>
"""
...
def main() -> None:
    ...
    parser.add_argument(
        '--json',
        type=Path,
        default=REPO_ROOT / 'agents' / 'data' / 'codebase-map' / 'backend-api-flow.json',
        help='Path to the api-flow JSON (default: agents/data/codebase-map/backend-api-flow.json)',
    )
    ...
    if not args.json.is_file():
        sys.exit(
            ...
            f'  cd agents/packages/api-flow && npm run extract'
        )
    data = json.load(args.json.open())
    out_dir = REPO_ROOT / 'agents' / 'data' / 'knowledge-graphs' / f'{data["package"]}-api-flow'

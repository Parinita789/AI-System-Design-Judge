...
Usage:
  agents/graphify/build-mermaid.py <package> [--out <path>]

Examples:
  agents/graphify/build-mermaid.py backend
  agents/graphify/build-mermaid.py frontend --out /tmp/frontend.md
"""
...
def build_mermaid(
    ...
    out.append(
        ...
        'Generated from `agents/codebase-map/' + package + '.json` '
        '(no LLM calls). See `agents/graphify/build-mermaid.py`.'
    )
    out.append('')
    out.append(
...
def build_mermaid_html(
    ...
    Matches the rest of agents/graphify/ where output is HTML."""
    mermaid_text, stats = build_mermaid_block(
        mapper_data, excludes=excludes, hide_infra=hide_infra, layout=layout,
    )
    package = stats['package']
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            'agents/graphify/<package>-module-level/MODULE_RELATIONSHIPS.md'
        ),
    )
    parser.add_argument(
        '--include-entry',
    ...
    md_path = args.out or (
        REPO_ROOT / 'agents' / 'graphify' / f'{args.package}-module-level'

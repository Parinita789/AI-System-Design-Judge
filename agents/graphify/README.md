...
graphs for this monorepo, plus seven small helper scripts that turn
graphify's raw output into views that are actually browsable.

Companion to [`agents/mapper/`](../mapper/) and
[`agents/codebase-map/`](../codebase-map/): the mapper gives you
plain-English module summaries; graphify gives you the graph
...
agents/graphify/map-module.sh evaluations

# Build the package-wide module-relationship views (free; no LLM)
agents/graphify/build-mermaid.py backend          # static Mermaid for GitHub/PRs
agents/graphify/build-neighborhoods.py backend    # per-module 1-hop neighborhoods
agents/graphify/build-explorer.py backend         # ← click-around HTML, the daily-driver

open agents/graphify/backend-module-level/MODULE_EXPLORER.html
```
...
| `build-mermaid.py <pkg>` | `agents/codebase-map/<pkg>.json` | `<pkg>-module-level/MODULE_RELATIONSHIPS.{md,html}` | no |
| `build-neighborhoods.py <pkg>` | `agents/codebase-map/<pkg>.json` | per-module `MODULE_NEIGHBORHOOD.{md,html}` | no |
| `build-explorer.py <pkg>` | `agents/codebase-map/<pkg>.json` | `<pkg>-module-level/MODULE_EXPLORER.html` | no |
...
```bash
agents/graphify/build-mermaid.py backend
open agents/graphify/backend-module-level/MODULE_RELATIONSHIPS.md
```
...
agents/graphify/map-module.sh evaluations
# Then open ONE of:
open agents/graphify/evaluations/graphify-out/GRAPH_TREE_high-level.html  # behavioral files
open agents/graphify/evaluations/graphify-out/GRAPH_TREE_files.html       # all files
open agents/graphify/evaluations/graphify-out/GRAPH_REPORT.md             # text summary
```
...
```bash
agents/graphify/build-neighborhoods.py backend
# Then for any module:
open agents/graphify/evaluations/MODULE_NEIGHBORHOOD.html
```
...
```bash
agents/graphify/build-unified.py backend --level high
open agents/graphify/backend-unified/graphify-out/GRAPH_TREE_unified-high.html
```
...
| Plain-English summary | `agents/codebase-map/<pkg>.md` |
...
- `agents/codebase-map/*.json` (mapper JSON sidecars)
...
`agents/graphify/map-module.sh` is the convenience wrapper: extract
...
- A previously-run mapper output at `agents/codebase-map/<package>.json`
  for any of the `build-*.py` scripts. Regenerate with:
  ```bash
  node agents/mapper/dist/index.js --no-with-llm --json \
